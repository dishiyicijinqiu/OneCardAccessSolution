...
namespace FengSharp.OneCardAccess.Common
{
    ...
    public class ClassValueCopier
    {
        ...
        public static void CopyArray<D, S>(List<D> destination, List<S> source)
        {
            if (destination == null || source == null)
                return;
            if (destination.Count != source.Count) throw new Exception("与目标长度不一样");
            for (int i = 0; i < destination.Count; i++)
            {
                destination[i] = Activator.CreateInstance<D>();
